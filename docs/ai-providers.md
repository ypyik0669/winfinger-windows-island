...
自建接口和中转网关上的模型名跟 OpenAI 官方那几个基本对不上，靠猜的结果就是每次动作都报
"模型或地址不存在"。两个地方都能直接问接口要列表（`GET {BaseUrl}/models`）：

- 功能设置 → AI → 模型右边的**「获取列表」**：拉到的模型填进下拉框，选一个即可。这个模型给
  翻译 / 总结 / `actions.json` 里的动作用。
- 对话页标题栏的**模型胶囊**：点开就是同一份列表。选中的模型对当前会话生效，同时成为之后
  「新建对话」的默认模型（写进"对话模型"设置）；选「跟随设置」则清掉本会话的指定，回到设置里的模型。

拉不到列表不影响手填——有些网关没实现 `/models`，照抄后台给的名字即可。
...
对话相关的三个设置在功能设置的「AI 对话」分区：对话模型（留空跟随上面的模型；在对话页选过模型后这里会自动填上）、系统提示词（留空用内置的，
...
## 换模型时先跑一遍 `Scripts/model-bench.py`

判断"这个模型到底能不能用"比测连接更实际：脚本按公开基准家族（MMLU / C-Eval / HellaSwag /
GPQA / GSM8K / ARC-AGI / HumanEval / SciCode 风格）各取一两道代表题，本地自动判分，
同时记首字延迟、总耗时和输出速度。它不是复刻官方榜单，只是换模型 / 换网关时的横向对照。
...
`--only mmlu,gsm8k` 只跑指定题目；`--no-exec` 跳过会执行模型返回代码的编程题
（那类题在独立子进程里带 10 秒超时执行，仍然是在本机跑陌生代码，只对信任的接口开）。
