...
| AI 对话 | 面板第六页「对话」：多轮聊天、流式输出、随时停止；左侧会话列表可重命名 / 删除，历史存在本地 `chat.json`，重启还在。回复按轻量 markdown 渲染（标题 / 列表 / **粗体** / 行内代码 / 代码块带复制按钮）。剪贴板条目右键「发送到 AI 对话」把内容带进输入框（图片先走离线 OCR，只发文字），结果抽屉里的 AI 输出可以「继续追问」接着聊。用的是同一套 OpenAI 兼容配置，`Ctrl+6` 直达，`Ctrl+N` 新建对话 |
