...
屏幕顶部常驻一个 iOS 灵动岛风格的 Liquid Glass 玻璃胶囊，实时显示网速与内存；点击弹性展开为六页面板：**剪贴板历史 / 媒体控制 / 便利贴 / 快捷键 / 番茄钟 / AI 对话**。
...
| AI 对话 | 面板第六页「对话」：多轮聊天、流式输出、随时停止；左侧会话列表可重命名 / 删除，历史存在本地 `chat.json`，重启还在。回复按轻量 markdown 渲染（标题 / 列表 / **粗体** / 行内代码 / 代码块带复制按钮）。剪贴板条目右键「发送到 AI 对话」把内容带进输入框（图片先走离线 OCR，只发文字），结果抽屉里的 AI 输出可以「继续追问」接着聊。用的是同一套 OpenAI 兼容配置；标题栏的模型胶囊可以直接从接口拉取模型列表并切换，选中的模型对当前会话生效并成为之后新建会话的默认值。`Ctrl+6` 直达，`Ctrl+N` 新建对话 |
...
| 功能设置 | 托盘 → **功能设置…**：三个全局快捷键的捕获与冲突检测（先试注册成功才落盘）、OCR 语言选择与语言包安装入口、AI 的 BaseUrl / 模型（可从接口一键拉取可用模型列表）/ 目标语言 / 超时 / API Key（DPAPI 加密）与连接测试、动作目录状态与"打开 / 重新加载 / 恢复默认" |
...
| 省电模式 | 外观设置 → 性能：关闭呼吸光、强调边框脉冲、封面辉光与频谱，网速 / 内存改为 2 秒一采，动态玻璃取景降到 2.5 帧/秒。实测（12 核机器，单核口径）：待机 2–4%，展开面板普通模式 15–20%、省电模式 2–6%；低配电脑建议再把背景切成「纯色」 |

## 交互

- **点击胶囊** 展开 / **Esc** 或 **点击面板外** 收起（锁上后点外面不收起）
- 展开后左上角 **WinFinger** 可收起；顶部标签和锁之间的空白：按住可以拖出或移动面板，没锁时轻点会收起
- **Ctrl+1..6** 切换页面（剪贴板/音乐/便利贴/快捷键/番茄钟/AI 对话），便利贴页与对话页 **Ctrl+N** 新建
- 点内存环可打开任务管理器
- 托盘图标：打开、外观（纯黑 / Liquid Glass / 外观设置…）、**功能设置…**、收起位置（顶部 / 悬浮）、暂停剪贴板、清空历史、岛背景、开机自启、退出
- 托盘 → **外观设置…**：岛背景三模式（动态玻璃 / 任意纯色 / 自定义图片），HSV 取色器 + Hex 输入、图片暗化、玻璃暗度/饱和度、光效开关、**省电模式**，全部实时预览并记忆；纯色/图片模式下取景完全停止
...
| `Ctrl+1..6` | 展开状态下切换六个页面 | 否（固定） |
| `Ctrl+N` | 便利贴页 / 对话页新建 | 否（固定） |

> **隐私**：OCR 与二维码识别完全离线，不联网、不上传任何内容；AI 功能只在你主动触发翻译 / 总结 / 对话时，把相应文本发送到功能设置里配置的接口地址（对话会带上本会话最近的几轮，字符预算可调）（默认 OpenAI，可换成本地部署的 Ollama 等），不会主动上传其他数据；API Key 用 Windows DPAPI（CurrentUser）加密后落盘，只有同一台电脑同一个 Windows 账户能解密。
>
...
| `WINFINGER_AUTOEXPAND=1..6` | 启动即展开到第 N 页 |
